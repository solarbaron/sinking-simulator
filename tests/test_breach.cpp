...
void testOverlappingSubdivision() {
    ...
    s.gyradii = {7.0, 28.0, 28.0};
    s.initialise(0.0);
    // Ship::validate() reports it now. It did not when this test was written --
    // its only overlap instrument was "do the compartments total more than the
    // hull", and a ship is ~89% subdivided, so one space can sit wholly inside
    // another and the sum never come near. A pairwise bounding-box check was
    // added when this test found the reference ferry doing exactly that.
    expectTrue("an overlapping subdivision is reported by validate()", [&] {
        for (const std::string& problem : s.validate())
            if (problem.find("overlap") != std::string::npos) return true;
        return false;
    }());
    ...
    // The reference ferry used to do exactly this and no longer does. When this
    // test was written, `fwd_hold_p` was authored y = 0..20 m while
    // `wing_tank_fwd_p` was 8..20 m over the same length and height, so the tank
    // sat wholly inside the hold and 217 m3 of her flooded twice. The forward
    // holds now stop at 8 m, as the aft ones always did.
    Ship ferry = game::buildFerry();
    ferry.initialise(0.0);
    expectEqual("the ferry's forward wing tank is its own space",
                spaceAt(ferry, {32, 9, 4}), ferry.findCompartment("wing_tank_fwd_p"));
    expectEqual("as the aft one, authored on the same plan, always was",
                spaceAt(ferry, {-24, 9, 4}), ferry.findCompartment("wing_tank_aft_p"));
    expectEqual("and the forward hold no longer reaches out under it",
                spaceAt(ferry, {32, 6, 4}), ferry.findCompartment("fwd_hold_p"));
    expectTrue("so the reference ship now validates clean", ferry.validate().empty());
