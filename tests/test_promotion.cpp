...
void testTheCellBudgetBindsAndTheCostIsMeasured() {
    ...
    // **`GasCriterion::coreSecondsPerCell` is deliberately not asserted against
    // these, and the attempt to do it is worth recording.** Bracketing the constant
    // between `coarsest` and `finest` holds on an ordinary build -- 1.55e-6 at 168
    // cells and 1.16e-5 at 1456, with the constant at 1.0e-5 between them -- and
    // fails under **both** sanitizers, which is where it was caught. Instrumentation
    // multiplies a timed solve by about ten and leaves the constant where it is, so
    // both bounds rise past it together. Taking a minimum over three attempts
    // defends against contention and does nothing at all against that.
    //
    // So it is the wall-clock assertion `CLAUDE.md` records as having produced false
    // kills before, arrived at from a new direction. What survives instrumentation
    // is the *shape*: the ratio asserted just above, which scales with the
    // measurement on both sides. The constant stays an extrapolation with a measured
