...
Component ids are assigned on first use, so their numeric values depend on which
type is touched first. Nothing may depend on those values; archetypes sort by id
only to get a canonical key. Stable cross-run ids are a job for the reflection
pass, and until it exists this is a known limit on cross-build reproducibility.
